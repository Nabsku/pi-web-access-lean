...
const COMMAND_ENVIRONMENT_NAMES = [
	"HOME", "USER", "LOGNAME", "PATH", "LANG", "LC_ALL", "LC_CTYPE", "TERM", "TMPDIR",
	"XDG_CONFIG_HOME", "XDG_RUNTIME_DIR", "DBUS_SESSION_BUS_ADDRESS", "SSH_AUTH_SOCK",
	"WSL_DISTRO_NAME", "WSL_INTEROP",
] as const;

export type CredentialFailureCategory =
	| "invalid-source" | "command-failed" | "command-timeout" | "command-aborted"
	| "command-empty" | "command-invalid-output" | "command-output-too-large" | "environment-empty";
...
export class CredentialResolutionError extends Error {
	...
	constructor(provider: string, category: CredentialFailureCategory) {
		super(`${provider} credential resolution failed: ${category === "command-aborted" ? "aborted" : category}`);
...
export interface CredentialCommandResult { stdout: string | Buffer; }
...
export type CredentialCommandRunner = (command: string, options: CredentialCommandOptions) => Promise<CredentialCommandResult>;
...
export function hasCredentialSource(options: CredentialOptions): boolean {
	...
	return source?.startsWith("!") === true || source?.startsWith("$") === true
		|| normalize(options.environmentValue) !== null || source !== null;
}
...
export async function resolveCredential(options: CredentialOptions): Promise<string | null> {
	...
	if (source?.startsWith("!")) {
		...
		if (Buffer.byteLength(stdout, "utf8") > MAX_CREDENTIAL_BYTES) throw new CredentialResolutionError(options.provider, "command-output-too-large");
		const value = stdout.trim();
		if (!value) throw new CredentialResolutionError(options.provider, "command-empty");
		if (/[\0-\x1f\x7f]/.test(value)) throw new CredentialResolutionError(options.provider, "command-invalid-output");
		return value;
	}
	if (source?.startsWith("$") && escapedSource(source) === null) {
